...
/**
   This client prompts the user for one line of text,
   sends that line to the server, and then reads the
   server's response.
<p>
   Since this version of the client prompts the user for
   the line of input to send to the server, we delay this
   client's input, and therefore block the server (since
   the server is single threaded).
<p>
   Or we can terminate this client after it has connected
   to the server and see how the server reacts.
<p>
   Also, try connecting this client to EchoServer_v2.java,
   which expects two lines from each client. What happens?
<p>
...
public class EchoClient_v1b
{
   ...
   */
   public static void main (String[] args)
   {
      ...
      final String clientMessage = message + " (from pid = " + pid + ")";
      out.println(clientMessage);
      out.flush(); // Make sure that the request is sent.
      System.out.println("CLIENT: Message sent to the server: " + "\"" + clientMessage + "\"");
