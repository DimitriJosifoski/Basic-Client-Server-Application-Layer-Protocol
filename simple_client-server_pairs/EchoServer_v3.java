...
/**
   This is an echo server that echoes an arbitrary
   number of text lines.
<p>
   This server reads and echoes all the input lines
   from a client. After this server detects the
   end-of-stream condition on a client connection,
   this server sends one last summary message back
   to the client.
<p>
   This echo server will show you everything that a web
   browser sends to a web server. Use the following
   web address in your browser's address bar.
<p>
      localhost:5000
<p>
   Or use one of the fetch URL clients from the "network_clients" folder.
<p>
    > java  Retrieve_URL_HttpClient  http://localhost:5000/
<p>
   ...
   */
   public static void main (String[] args)
   {
      ...
      while (true) // Serve multiple clients.
      {
         ...
         // Read all the client's input lines.
         try
         {
            int inputCounter = 0;
            String input;
            while ( (input = in.readLine()) != null ) // Read until "eof".
            {
               ++inputCounter;
               // Log the client's input to the server's console.
               System.out.println("SERVER: Client " + clientCounter
                                + ": Message received: \"" + input + "\"");

               // Echo the client's input back to the client.
               out.println("You are client " + clientCounter + ": " + input);
               out.flush(); // Make sure that the response is sent.
            }

            // Send the client one more response.
            out.println("You are client " + clientCounter
                      + ": Thank you for your " + inputCounter + " inputs.");
            out.flush(); // Make sure that the response is sent.

            System.out.println("SERVER: Client " + clientCounter
                             + ": Done. (Received "
                             + inputCounter + " messages.)");
